...
<div class="featured-collection-section mt-100 home-section overflow-hidden">
    <div class="container">
        <div class="section-header text-center">
            {{-- <p class="section-subheading">WHAT'S NEW</p> --}}
            <h2 class="section-heading">SẢN PHẨM ĐANG GIẢM GIÁ</h2>
        </div>
        ...
        <div class="product-container position-relative">
            ...
            @php
                $config = \App\Models\Config::orderByDESC('id')->first();
                $sale = explode(",",$config->list_sale);
                $data2 = \App\Models\SanPham::whereIn('id', $sale)->get();
            @endphp
            @foreach ($data2 as $k => $v)
            <div class="new-item" data-aos="fade-up" data-aos-duration="700">
                <div class="product-card">
                    <div class="product-card-img">
                        <a class="hover-switch" href="/product/{{$v->slug_san_pham}}-post{{$v->id}}">
                            <img style="height: 360px; width: 100%;" class="secondary-img" src="{{ explode(",",$v->hinh_anh)[0] }}"
                                alt="product-img">
                            <img style="height: 360px; width: 100%;" class="primary-img" src="{{ explode(",",$v->hinh_anh)[0] }}"
                                alt="product-img">
                        </a>
                        @php
                            $arr    = ['Sale', 'Hot', 'New', ''];
                        ...
                        <div class="product-badge">
                            ...
                            <span class="badge-label badge-percentage rounded">-{{ number_format(($v->gia_ban - $v->gia_khuyen_mai) / $v->gia_ban * 100, 0, '.', ',') }}%</span>
                            @endif
                        </div>
                    </div>
                    <div class="product-card-details">
                        <h3 class="product-card-title">
                            <a href="/product/{{$v->slug_san_pham}}-post{{$v->id}}">{{ $v->ten_san_pham }}</a>
                        </h3>
                        <div class="product-card-price">
                            @if($v->gia_khuyen_mai > 0)
                            <span class="card-price-regular">{{ number_format($v->gia_khuyen_mai, 0, '.', ',') }} đ</span>
                            <span class="card-price-compare text-decoration-line-through">{{ number_format($v->gia_ban, 0, '.', ',') }} đ</span>
                            @else
                            <span class="card-price-regular">{{ number_format($v->gia_ban, 0, '.', ',') }} đ</span>
                            @endif
                        </div>
                    </div>
                </div>
